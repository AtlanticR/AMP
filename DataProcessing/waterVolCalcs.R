################################################################################
################################################################################
#### Calculating cumulative water volume analyzed for rarefaction curves
# This is a VERY fast coding approach because I am in a rush (again!)
# I cannot just use the PercZooIdentified and PercSampleCleaned from my zooplanktonCounts.R file
# That's because I had to enter 100% for both columns if there was a 5mm size fraction
# That messes things up!!!
# I need to remove those and calculate water volumes again
# This is basically the same code as zooplanktonCounts.R BUT I need to comment out lines that look like this:
# mutate(PercSampleCleaned = replace_na(PercSampleCleaned, 1)) %>%
# Those were the lines that made those values equal to 100 for the 5mm fraction

# So, I am excluding those, then extract the following columns from each dataset:
# "sampleCode", "PercZooIdentified", "PercSampleCleaned", "waterVolume"
# I need these to do the rarefaction curves and adjust for cumulative water volume
...
mergeSpeciesMeta = function(metadata, speciesDataset) {
  mergedData = full_join(metadata, speciesDataset, by = c("flowcamCode" = "sample")) %>%
    ...
    group_by(flowcamCode, PercSampleCleaned, PercZooIdentified, newName, isCopepod, copepodType, facilityName, waterVolume, dataset, yearStart, monthStart, dayStart, myLabel, tidePhase, sampleCode, depthWaterM, productionType, target, samplingDesign, equipmentType, TowType, netMesh) %>% 
...
gulfMerge = mergeSpeciesMeta(gulfMetaRed, gulfAll) %>%
  ...
  filter(!is.na(PercZooIdentified)) %>% # remove the files with NA!!! These are the 5mm size fraction
  group_by(sampleCode) %>%
  slice(1) %>% # only select the 1st sampleCode entry from the dataframe. This will have all I need (below)!
  select(c("sampleCode", "PercZooIdentified", "PercSampleCleaned", "waterVolume"))
...
nlMerge = mergeSpeciesMeta(nlMetaRed, nlAll) %>%
  ...
  rename(class = newName) %>%
  filter(!is.na(PercZooIdentified)) %>%
  group_by(sampleCode) %>%
  slice(1) %>% # only select the 1st sampleCode entry from the dataframe. This will have all I need (below)!
  select(c("sampleCode", "PercZooIdentified", "PercSampleCleaned", "waterVolume"))
...
# Now, right it out to a file
export = rbind(gulfMerge, nlMerge, marMerge, pacMerge)
